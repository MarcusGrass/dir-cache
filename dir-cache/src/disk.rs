...
pub(crate) fn read_all_in_dir<F: FnMut(&Path, &Metadata) -> Result<()>>(path: &Path, mut func: F) -> Result<()> {
    for e in  std::fs::read_dir(path).map_err(|e| Error::ReadContent("Failed to read dir", Some(e)))? {
        let entry = e.map_err(|e| Error::ReadContent("Failed to read dir entry", Some(e)))?;
        let entry_path = entry.path();
        let entry_md = entry.metadata().map_err(|e| Error::ReadContent("Failed to read entry metadata", Some(e)))?;
        func(&entry_path, &entry_md)?;
    }
    Ok(())
}

pub(crate) fn read_subdirs(path: &Path) -> Result<impl Iterator<Item=Result<Option<PathBuf>>>> {
    let it = std::fs::read_dir(path).map_err(|e| Error::ReadContent("Failed to read dir", Some(e)))?
        .flat_map(|entry| {
            let entry = match entry.map_err(|e| Error::ReadContent("Failed to read dir entry", Some(e))){
                Ok(e) => e,
                Err(e) => {
                    return std::iter::once(Err(e));
                }
            };
            let entry_md = match entry.metadata().map_err(|e| Error::ReadContent("Failed to read entry metadata", Some(e))) {
                Ok(md) => md,
                Err(e) => {
                    return std::iter::once(Err(e));
                }
            ...
            if entry_md.is_dir() {
                std::iter::once(Ok(Some(entry.path())))
            } else if entry_md.is_file() {
                std::iter::once(Ok(None))
            } else {
                std::iter::once(Err(Error::ReadContent("Expected dir or file in subdir, found something else", None)))
            }
        });
    Ok(it)
}
    ...
    match exists(path)? {
        FileObjectExists::No => {
            std::fs::create_dir(path)
                .map_err(|e| Error::WriteContent("Failed create dir if missing, failed create", Some(e)))
        }
        FileObjectExists::AsDir => {
            Ok(())
        }
        FileObjectExists::AsFile => {
            Err(Error::ReadContent("Failed create dir, expected nothing or an already existing dir, found a file", None))
        }
    }
...
pub(crate) fn exists(path: &Path) -> Result<FileObjectExists> {
    match std::fs::metadata(path) {
        ...
        Err(e) if e.kind() == ErrorKind::NotFound => {
            Ok(FileObjectExists::No)
        }
        Err(e) => {
            Err(Error::ReadContent("Failed to read metadata to check path existence", Some(e)))
        }
    }
}
...
pub(crate) fn read_metadata_if_present(path: &Path) -> Result<Option<String>> {
    match std::fs::read_to_string(path) {
        ...
        Err(e) => {
            Err(Error::ReadContent("Failed to read metadata", Some(e)))
        }
    }
}
pub(crate) fn read_raw_if_present(path: &Path) -> Result<Option<Vec<u8>>> {
    match std::fs::read(path) {
        ...
        Err(e) => {
            Err(Error::ReadContent("Failed to read file", Some(e)))
        }
    }
}
...
pub(crate) fn ensure_removed_file(path: &Path) -> Result<()> {
    if let Err(e) = std::fs::remove_file(path) {
        if e.kind() != ErrorKind::NotFound {
            return Err(Error::DeleteContent("Failed to ensure file was removed", Some(e)));
        }
    }
    Ok(())
}
