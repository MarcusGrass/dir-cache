...
fn create_only_if_exists_fail_if_not_exists() {
    ...
    let Err(e) = DirCacheOpts::default()
        .open(&doesnt_exist, CacheOpenOptions::new(DirOpen::OnlyIfExists, true)) else {
...
fn create_only_if_exists_works_if_exists() {
    ...
    DirCacheOpts::default()
        .open(&exists, CacheOpenOptions::new(DirOpen::OnlyIfExists, true)).unwrap();
}
...
fn create_if_missing_will_create() {
    ...
    let _ = DirCacheOpts::default()
        .open(&doesnt_exist, CacheOpenOptions::new(DirOpen::CreateIfMissing, true)).unwrap();
...
fn assert_dir_at(path: &Path) {
    let p = check_path(path)
        .expect("Expected dir, found nothing");
